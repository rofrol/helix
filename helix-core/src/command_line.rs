...
use std::{borrow::Cow, collections::HashMap, error::Error, fmt, ops, vec};
...
///
/// Each typable command defines a signature (usually with the help of `Signature::default` or
/// `Signature::raw`) at least to declare how many positional arguments it accepts. Command flags
/// are also declared in this struct. The `raw_after` option may be set optionally to avoid
/// evaluating quotes in parts of the command line (useful for shell commands for example).
#[derive(Debug, Clone, Copy)]
...
impl Signature {
    ...
    /// The command handles all of its input as-is with no quoting or flags.
...
/// to accept and this is checked before executing a command with `PromptEvent::Validate`, so
/// indexing into `Args` within a command's accepted range is safe.
#[derive(Debug)]
pub struct Args<'a> {
    signature: Signature,
